...
10. Once data has been smoothed, we can assemble a `sspm` model object, using one dataset of type biomass, one dataset of type predictor and (optionnaly) a dataset of type catch. If we want to use a 
...
11. Before fitting the model, we must split data into test/train with `spm_split`.
...
12. To fit the model, we might be interested in including lagged values. This is done with `spm_lag`.
...
14. Plotting the object produces a actual vs predicted plot (with TEST/TRAIN data highlighted.
